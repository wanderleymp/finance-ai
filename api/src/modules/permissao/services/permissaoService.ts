...
import { GrupoPermissaoService } from './grupoPermissaoService';
...
export class PermissaoService implements IPermissaoService {
  ...
  constructor(
    ...
    @Inject(forwardRef(() => GrupoPermissaoService))
    private readonly grupoPermissaoService: GrupoPermissaoService
  ) {}
   ...
   */
  async buscarPermissaoPorId(id: string): Promise<any> {
  ...
  async verificarPermissao(
    ...
    const temPermissaoDireta = await this.verificarPermissaoDireta(
      usuarioId,
      moduloId,
      submduloId,
      recursoId,
    ...
    // Verificar permissão por grupo
    const temPermissaoPorGrupo = await this.grupoPermissaoService.verificarPermissaoPorGrupo(
      usuarioId,
      moduloId,
      submduloId,
      recursoId,
    ...
    return temPermissaoPorGrupo;
  }
  
  /**
   * Verifica se um usuário tem permissão direta (sem considerar grupos)
   * @param usuarioId ID do usuário
   * @param moduloId ID do módulo
   * @param submduloId ID do submódulo (opcional)
   * @param recursoId ID do recurso (opcional)
   ...
   * @returns Promise com boolean indicando se o usuário tem permissão
   */
  async verificarPermissaoDireta(
    usuarioId: string,
    moduloId: string,
    submduloId?: string,
    recursoId?: string,
   ...
   * @returns Promise com array de permissões do usuário
   */
  async listarPermissoesPorUsuario(usuarioId: string): Promise<any[]> {
    // Este método é um alias para obterPermissoesUsuario para manter compatibilidade
    return this.obterPermissoesUsuario(usuarioId);
  }
